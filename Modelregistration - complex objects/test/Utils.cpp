...
void drawQuestion(cv::Mat image, cv::Point3f point, cv::Scalar color)
{
	...
	cv::putText(image, text, cv::Point(25, 25), fontFace, fontScale, color, thickness_font, 8);
}
...
void drawCounter(cv::Mat image, int n, int n_max, cv::Scalar color)
{
	...
	cv::putText(image, text, cv::Point(25, 200), fontFace, fontScale, color, thickness_font, 8);
}

void drawCounter2(cv::Mat image, int n, int n_max, cv::Scalar color)
{
	std::string n_str = IntToString(n);
	std::string n_max_str = IntToString(n_max);
	std::string text = n_str + " of " + n_max_str + " pictures";
	cv::putText(image, text, cv::Point(25, 200), fontFace, fontScale, color, thickness_font, 8);
}
...
//initialize the intrasic camera matrix
cv::Mat initCam(const double params[])
{
	cv::Mat _A_matrix;
