...
{
	cv::Mat points3d_mat;
	std::vector<cv::Point3f> list_points_3D_new;
	cv::Mat descriptors;
	...
	if (list_points3d_in_.empty())
		points3d_mat.copyTo(list_points3d_in_);
	else
		points3d_mat.copyTo(list_points_3D_new);
		list_points3d_in_.insert(list_points3d_in_.end(),list_points_3D_new.begin(),list_points_3D_new.end());

	if (descriptors_.empty())
		descriptors.copyTo(descriptors_);
	else
		vconcat(descriptors_, descriptors, descriptors_);
